...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        // Fetch testimonials using useAdminApi
        const { data: testimonialsData, error: testimonialsError } = await get<any[]>("/testimonials/admin");
        if (testimonialsData) {
        ...
        // Fetch messages using useAdminApi
        const { data: messagesData, error: messagesError } = await get<any[]>("/contact/messages");
        if (messagesData) {
  ...
  const dashboardItems = [
    ...
    },
    {
      ...
      description: t("viewPortfolioDesc"),
      icon: Eye,
      route: "/",
    },
  ];
                  ...
                  {educationList.map((edu) => (
                    ...
                    >
                      <div className="font-medium text-gray-900 dark:text-gray-100">
                        {edu.degreeEn}
                        {edu.fieldEn
                          ? ` ${tEdu("degreeFieldPreposition")} ${edu.fieldEn}`
                          : ""}
                      </div>
                      <div className="text-sm text-gray-600 dark:text-gray-400">
                        {edu.institutionEn}
