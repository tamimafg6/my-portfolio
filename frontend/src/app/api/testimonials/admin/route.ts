...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log(
        "[Admin Testimonials] AUTH_SERVICE_URL:",
        process.env.AUTH_SERVICE_URL,
      );
      console.log("[Admin Testimonials] AUTH_URL:", process.env.AUTH_URL);
      console.log(
        "[Admin Testimonials] NEXT_PUBLIC_AUTH_URL:",
        process.env.NEXT_PUBLIC_AUTH_URL,
      );
      console.log(
        "[Admin Testimonials] Cookies present:",
        cookies ? "Yes" : "No",
      );
      console.log(
        "[Admin Testimonials] Cookie value:",
      ...
      console.log(
        "[Admin Testimonials] Token response status:",
        tokenRes.status,
      );
      console.log(
        "[Admin Testimonials] Token response headers:",
      ...
      if (tokenRes.ok) {
        ...
        if (authToken) {
          console.log(
            "[Admin Testimonials] Token retrieved successfully, length:",
            authToken.length,
          );
        } else {
          console.error(
            "[Admin Testimonials] No token in response:",
            tokenData,
          );
        }
      } else {
        const errorText = await tokenRes
          .text()
          .catch(() => "Could not read error");
        console.error(
          "[Admin Testimonials] Token request failed:",
          tokenRes.status,
          errorText,
        );
      ...
      console.error(
        "[Admin Testimonials] Error getting auth token:",
        error?.message || error,
      );
    ...
    if (!authToken) {
      console.error(
        "[Admin Testimonials] No auth token available after retrieval attempt",
      );
      return NextResponse.json(
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        {
          error: `Failed to fetch testimonials: ${res.status}`,
          details: errorText,
        },
        {
          status: res.status,
    ...
    console.log(
      "[Admin Testimonials] Data received:",
    ...
    console.error(
      "[Admin Testimonials] Error in route handler:",
      error?.message || error,
    );
